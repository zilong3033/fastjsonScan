...
def check_url(ip, port):
    ...
    if port in (443,8443,6443,7443,18443,9443,17443,1443):
...
def cScan(pre_ip, ports):
    print("[+]Checking %s0 ..." % (pre_ip))
    li=[]
    for port in ports:
        s = scan(ip=pre_ip, port=int(port))
        li.append(gevent.spawn(s.run))
    gevent.joinall(li)
    print("ipc %s is over in %.2f seconds!" % (pre_ip, time.time() - start_t))
    sys.exit(0)

def xray_webscan():
    xray_path="D:\\Program Files\\xray\\xray_windows_386.exe"
    url_file="urls.txt"
    xray_output="0223.html"
...
if __name__ == '__main__':
    parser = optparse.OptionParser(
        'usage: %prog [options] target\nexample:\n\t %prog ip.txt\n\t%prog -i 192.168.1.1/24', version="%prog 1.0.9.10",
        description="default scan HttpTitle")
    parser.add_option('-i', '--ips', dest='ips', type='string', help='192.168.1.1/24,192.168.2.1/24')
    ...
    parser.add_option('-p', '--ports', dest='ports',
                      default="80,443,8080,81,82,83,84,85,88,86,87,89,8000,8001,8002,8010,8050,808,8880,8888,8443,8081,8082,8083,8161,8090,9443,7002,7001,8088,18080,18088,8082,8083,18443,9090,9080,9081,9082,6443,7443,8001,8002,9000,9001,8082",
                      type='string',
                      help='the ports you want to scan,default ports (80,443,8080,81,82,83,84,85,88,8000,8010,8050,808,8880,8888,8443,8081,8161,8090,9443,7002,7001,8088,18080,18088,8082,8083,18443,9090,9080,6443,7443)')
    ...
    if options.ips:
        ...
        if str(ips).find(",") != -1:
            ip_list = ips.split(",")
            for i in ip_list:
                m = re.compile('(\d+\.\d+\.\d+\.)').findall(i)
                if m: l1.append(m[0])
            print(l1)
        else:
            m = re.compile('(\d+\.\d+\.\d+\.)').findall(options.ips)
            l1 = m
    else:
    ...
    #端口处理
    ports = options.ports
     #e为企业级web端口，d为默认web端口，s为精简web端口，扫描器默认s
    if options.mode=="s":
    ...
    if options.mode=="e":
        pass
        #ports="80-100,443,1000-10000,10001,10003,10005,10030,10035,10162,10243,10250,10255,10332,10333,10443,10554,11001,11211,11300,11310,11371,11965,12000,12300,12345,12999,13579,13666,13720,13722,14000,14147,14265,14443,14534,15000,16000,16010,16030,16443,16922,16923,16992,16993,17000,17443,17988,18000,18001,18080,18081,18086,18245,18246,18264,18443,19150,19888,19999,20000,20332,20547,20880,22105,22335,23023,23424,25000,25010,25105,25565,26214,26470,27015,27016,27017,28017,28080"
    if "-" in ports:
        portl=[]
        ports = ports.split('-')
    ...
    if len(ports)<50:
        ...
        if len(ports)<101:
            ...
            print("大于250端口，未优化，由于操作系统并发限制。")

    # 加入线程池，每次扫描
    threadPoolList = []
    THREADS_NUM=n

    tasks=[l2[i:i+n] for i in range(0,len(l2),n)]  #每n个为一个task
